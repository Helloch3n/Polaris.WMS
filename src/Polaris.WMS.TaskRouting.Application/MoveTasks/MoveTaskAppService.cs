...
namespace Polaris.WMS.TaskRouting.Application.MoveTasks
{
        ...
        : ApplicationService, IMoveTaskAppService
    {
        ...
        {
            // 调用领域服务
            await moveTaskManager.CompleteMoveTaskAsync(input.TaskId, input.ScannedLocationCode);
        }

        public async Task<PagedResultDto<MoveTaskDto>> GetListAsync(MoveTaskSearchDto input)
        {
            var queryable = await taskRepository.GetQueryableAsync();

            if (!string.IsNullOrWhiteSpace(input.TaskCode))
            {
                queryable = queryable.Where(x => x.TaskNo == input.TaskCode);
            }
            ...
            var dtos = ObjectMapper.Map<List<MoveTask>, List<MoveTaskDto>>(items);

            return new PagedResultDto<MoveTaskDto>(totalCount, dtos);
        }
    }
}