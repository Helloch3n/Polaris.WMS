using Polaris.WMS.InventoryManage.Application.Contracts.Containers;
...
namespace Polaris.WMS.InventoryManage.Application.Containers
{
    public class ContainerAppService(
        IRepository<Container, Guid> repository,
        ...
        :
            CrudAppService<Container, ContainerDto, Guid, ContainerSearchDto, CreateUpdateContainerDto>(repository),
            IContainerAppService
    {
        /// <summary>
        /// 获取可搬运盘具列表（按来源仓库过滤未锁定盘具）。
        /// </summary>
        public async Task<List<MovableContainerDto>> GetMovableReelsAsync(Guid sourceWarehouseId)
        {
            ...
            {
                return new List<MovableContainerDto>(); // 如果仓库下没库位，直接返回
            }
            ...
            if (reels.Count == 0)
            {
                return new List<MovableContainerDto>();
            }

            // 第三步：批量查询盘具下库存 (本地模块，合法)
            var containerIds = reels.Select(x => x.Id).ToList();
            var inventoryQuery = await inventoryRepository.GetQueryableAsync();
            var inventories = await AsyncExecuter.ToListAsync(inventoryQuery.Where(x => containerIds.Contains(x.ContainerId)));
            ...
            // 第五步：O(1) 内存组装返回 DTO
            var result = reels.Select(container =>
            {
                ...
                var currentLocationId = container.CurrentLocationId ?? Guid.Empty;
                var currentLocationCode = container.CurrentLocationId.HasValue &&
                                          locationMap.TryGetValue(container.CurrentLocationId.Value, out var code)
                    ? code
                    : string.Empty;

                return new MovableContainerDto
                {
        ...
        /// 创建盘具并返回结果（不补充库位编码）。
        /// </summary>
        public override async Task<ContainerDto> CreateAsync(CreateUpdateContainerDto input)
        {
            ...
            var entity = await containerManager.CreateAsync(
                id,
                input.ContainerCode,
                input.Name,
                input.Size,
            ...
            return ObjectMapper.Map<Container, ContainerDto>(entity);
        }
        ...
        /// 更新盘具基础信息，并按需更新库位。
        /// </summary>
        public override async Task<ContainerDto> UpdateAsync(Guid id, CreateUpdateContainerDto input)
        {
            ...
            var containerCode = string.IsNullOrWhiteSpace(input.ContainerCode)
        ...
        /// </summary>
        public override async Task<PagedResultDto<ContainerDto>> GetListAsync(ContainerSearchDto input)
        {
            var queryable = await Repository.GetQueryableAsync();

            if (!string.IsNullOrWhiteSpace(input.ContainerCode))
            {
                queryable = queryable.Where(x => x.ContainerCode == input.ContainerCode);
            }
            ...
            // 1. 实体映射
            var dtos = items.Select(x => ObjectMapper.Map<Container, ContainerDto>(x)).ToList();
            // 2. 补充跨聚合展示字段（库位/区域/仓库编码）
            await PopulateLocationDetailsAsync(dtos);

            return new PagedResultDto<ContainerDto>(totalCount, dtos);
        }

        /// <summary>
        /// 根据主键获取盘具实体。
        /// </summary>
        protected override async Task<Container> GetEntityByIdAsync(Guid id)
        {
        ...
        /// 获取单个盘具，并补充库位编码。
        /// </summary>
        public override async Task<ContainerDto> GetAsync(Guid id)
        {
            ...
            // 2. 实体映射
            var dto = ObjectMapper.Map<Container, ContainerDto>(entity);
        ...
        /// </summary>
        private async Task PopulateLocationDetailsAsync(List<ContainerDto> dtos)
        {
